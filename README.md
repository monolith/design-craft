...
- **`dc-art-director`** (developing-style) sets theme/feel and arbitrates; **`graphic-composition`, `perception-and-color`, `tufte-charting`, `typesetting`, `ux`, `brand-design`** are the specialist levers; **`brand-capture`** is conditional (only when adopting an existing brand). **`house-style-templates`** is a **reference-only skill** — a library of directional house-look templates the art-director draws from — **not an agent**.
