...
- **`dc-art-director`** (developing-style) sets theme/feel and arbitrates; **`dc-composition`, `dc-color`, `dc-charting`, `dc-typography`, `dc-layout`, `dc-branding`** are the specialist levers; **`dc-visual-style`** is the house hand; **`dc-brand-absorption`** is conditional (only when adopting an existing brand).
