...
## What you defer
- All colour/contrast/palette → `dc-color`. Type inside charts (tabular figures, ≥14px labels) → `dc-typography`. Where the chart sits on the page → `dc-layout`. Mood/atmosphere of the data section → `dc-art-director`.
...
When the art-director or composition wants a dramatic fill/shape that hides the real series, propose the Tufte form that keeps the drama **honestly** (e.g. range-frame, reference band, slopegraph). You own form; defer the colour of it to `dc-color`. One settled chart.
