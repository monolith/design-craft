...
## What you defer
- Colour science / contrast / palette → `perception-and-color`. Type selection/scale → `dc-typography`. Chart forms → `tufte-charting`. Page structure / nav / usability → `dc-layout`. Arrangement & gestalt execution → `dc-composition`. Identity/mark/personality → `dc-branding`. Absorbing an *existing* brand → `dc-brand-absorption` (use that instead of inventing when the brief is a facelift/reskin).
