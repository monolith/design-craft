...
## What you defer
- Colour science / contrast / palette → `perception-and-color`. Type selection/scale → `typesetting`. Chart forms → `tufte-charting`. Page structure / nav / usability → `ux`. Arrangement & gestalt execution → `graphic-composition`. Identity/mark/personality → `brand-design`. Absorbing an *existing* brand → `brand-capture` (use that instead of inventing when the brief is a facelift/reskin).
