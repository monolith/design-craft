...
- **Fully synthesised moving texture — pure `ffmpeg`, no image at all.** For a *screen* texture (animated static, flicker, blur, a phosphor field), synthesise it with `ffmpeg` filters: a `noise` source → colour/tint (`lutrgb`) → `gblur` → a time-varying `eq` brightness flicker (`eval=frame`) → `vignette`. e.g. a **green-CRT dead-channel**: dark-green base + strong temporal noise + green phosphor map + light blur + a ~3 Hz flicker. No still, no model — a controllable procedural loop. (Ken-Burns over a *generated* still is the sibling move → [[generative-imagery]].)
- **Layering a screen effect OVER footage (a "CRT monitor showing X").** To make real/generated footage read as a phosphor screen, **treat the footage AS the screen**: desaturate → colour-map (e.g. green phosphor) → static → gentle flicker → vignette. Do **NOT** screen-blend a coloured static *over* warm footage — green-over-warm accumulates red+blue and washes **purple**. For static that **moves** (not just flickers), add a second `noise` layer that regenerates (`allf=t`, temporal) **and** drifts (`crop` pan on an oversized source), blended **`lighten`** (screen accumulates channels → purple; lighten keeps the hue and adds bright grain). Web-playback musts: **`-movflags +faststart`** (moov atom to the front — without it a big clip stalls to **black**) and a **poster** frame; heavy static won't compress small — accept ~10–15 MB or trade static for size. *(Worked: green-CRT phosphor layered over a Ken-Burns push, all ffmpeg.)*
