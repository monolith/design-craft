...
**Don't go too literal.** A brief, concept, or theme *name* sets a **register and a feeling** — it is **not a list of motifs to depict**. Translating it literally — a "Ledger" theme built as accounting furniture, a "cloud" theme staged as weather clip-art, a name read as a genre the user never meant — produces **costume, not style**; the signature is the *form*, not the props. Take the register and the feeling from the intent, and deviate at the level of **structure and hand**, not by staging the concept's literal objects. **This holds for a *confirmed* concept too, not just a name:** even when the direction is locked, express it abstractly — a *Prism* is its geometry and colour-mix, not a drawn prism or a rainbow spectrum — and reach for the literal depiction **only when the user explicitly asks for it.** Default to the abstract and evocative; on-the-nose literalism is the recurring failure. When a name or word *seems* to name a genre, that's a **question for the user, not an instruction** (see the art-director's Step-1 rule against deriving direction from a name).
